...
@section('content')
    <div class="w-full font-['Manrope'] text-[#1e1f22]">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-4 border-b border-gray-100">
            <div>
                <a href="{{ route('admin.specialists.create') }}" class="text-xs uppercase tracking-wider text-[#7c7e8c] hover:text-[#ff5c8a] transition-colors duration-200 font-medium mb-2 inline-block">
                    ← Вернуться к выбору пользователя
                </a>
                <h1 class="text-2xl font-normal tracking-wider uppercase text-[#1e1f22] font-[Playfair_Display]">Шаг 2: Данные специалиста</h1>
                <p class="text-xs text-[#7c7e8c] font-light mt-1">
                    Вы назначаете мастером: <span class="font-normal text-[#ff5c8a]">{{ $user->last_name }} {{ $user->name }}</span> ({{ $user->email }})
                </p>
            </div>
        </div>

        {{-- ФОРМА ЗАПОЛНЕНИЯ (ПОЛНАЯ ШИРИНА И ОДИН СТОЛБЕЦ) --}}
        <main class="w-full">
                    ...
                    <div class="flex flex-col gap-6 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <textarea id="bio"
                                      ...
                                      class="block w-full rounded-xl border border-[#f1f1f5] bg-[#f8f8fa] focus:border-[#ff5c8a] focus:bg-white focus:ring-[#ff5c8a]/20 text-sm p-3 transition-all duration-200 outline-none font-light resize-y">{{ old('bio') }}</textarea>
                            @error('bio')
                            <p class="text-xs text-rose-500 font-light mt-1">⚠️ {{ $message }}</p>
                            @enderror
                        </div>
