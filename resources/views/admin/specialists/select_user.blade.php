...
@section('content')
    <div class="w-full font-['Manrope'] text-[#1e1f22]">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-4 border-b border-gray-100">
            <div>
                <h1 class="text-2xl text-[#1e1f22] font-[Playfair_Display]">Шаг 1: Выберите пользователя</h1>
        ...
        <div class="w-full bg-white p-4 rounded-3xl border border-[#f1f1f5] shadow-sm mb-6">
            <form action="{{ route('admin.specialists.create') }}" method="GET" class="flex flex-col sm:flex-row gap-3 w-full">
                ...
                <div class="flex gap-2 shrink-0">
                    <button type="submit" class="px-6 py-3 bg-[#1e1f22] hover:bg-black text-white text-xs tracking-wider  font-normal rounded-xl transition-all duration-200 cursor-pointer">
                        Найти
                    </button>
                    @if(request('search'))
                        <a href="{{ route('admin.specialists.create') }}" class="px-4 py-3 border border-gray-200 text-gray-500 hover:text-[#1e1f22] text-xs tracking-wider  font-normal rounded-xl transition-all duration-200 text-center flex items-center justify-center">
                            Сбросить
                        </a>
                    @endif
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tr class="border-b border-[#f1f1f5] text-[#7c7e8c] text-[10px] font-medium  tracking-wider bg-[#f8f8fa]">
                        <th class="px-6 py-4 text-sm  tracking-wider text-[#7c7e8c] font-medium">ФИО Пользователя</th>
                        <th class="px-6 py-4 text-sm  tracking-wider text-[#7c7e8c] font-medium">Телефон</th>
                        <th class="px-6 py-4 text-sm  tracking-wider text-[#7c7e8c] font-medium">Email</th>
                        <th class="px-6 py-4 text-sm  tracking-wider text-[#7c7e8c] font-medium text-right">Действие</th>
                    </tr>
                    </thead>
                        ...
                        <tr class="hover:bg-[#f8f8fa]/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.specialists.build', $user->id) }}"
                                   class="inline-flex items-center justify-center px-4 py-2 bg-[#ff5c8a] hover:bg-[#e04b75] text-white text-xs tracking-wider  font-normal rounded-xl transition-all duration-300 shadow-sm cursor-pointer whitespace-nowrap">
                                    Сделать мастером →
                                </a>
                            </td>
                        </tr>
