...
@section('content')
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-bold text-slate-800">Список услуг</h1>
        <a href="{{ route('admin.services.create') }}" class="px-4 py-2 bg-pink-500 hover:bg-pink-600 text-white font-bold rounded-xl text-sm transition-all shadow-sm">
            + Добавить услугу
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tr class="border-b border-slate-100 text-slate-400 text-xs font-bold uppercase bg-slate-50/50">
                ...
                <th class="px-6 py-3">Активность</th>
            ...
            @forelse($services as $service)
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 font-mono text-xs text-slate-400">{{ $service->description ?? '—' }}</td>

                    <td class="px-6 py-4 max-w-xs truncate text-slate-500">{{ $service->category->display_name}}</td>

                    <td class="px-6 py-4 max-w-xs truncate text-slate-500">{{ $service->active ? 'Активна':'Недоступна'}}</td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <a href="{{ route('admin.services.edit', $service->id) }}" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-lg transition-colors">
                            Изменить
                        </a>
                        <form action="{{ route('admin.services.destroy', $service->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Вы уверены?')">
                            @csrf
                            @method('DELETE')
