<x-app-layout title="Центр Ресниц | Оставить отзыв">
    ...
    <div class="w-full bg-cover bg-center pt-16 pb-16 relative" style="background-image: url('{{ asset('img/bg_main.png') }}');">
        ...
        <div class="max-w-7xl mx-auto px-6 text-center">
            <h2 class="text-3xl lg:text-4xl font-normal text-[#1e1f22]   mb-4 font-serif">
                Ваш отзыв
            </h2>
    ...
    <section class="max-w-2xl mx-auto pb-24 px-6 mt-8">
        <div class="bg-white p-8 lg:p-10 rounded-3xl border border-[#f1f1f5] shadow-sm">
            ...
            <div class="flex items-center gap-4 bg-[#fff0f3] p-4 rounded-2xl mb-8 border border-pink-100">
                ...
                <div>
                    <span class="text-sm text-pink-500 font-normal   block">Вы делитесь впечатлениями о мастере:</span>
            ...
            <form action="{{ route('reviews.store') }}" method="POST" x-data="{ rating: 5 }" class="space-y-6">
                ...
                <div class="space-y-2 text-center sm:text-left">
                    <label class="text-sm text-[#7c7e8c] font-light   block">Ваша оценка</label>
                ...
                <div class="space-y-2">
                    <label for="comment" class="text-sm text-[#7c7e8c] font-light   block">Комментарий</label>
                ...
                <div class="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-dashed border-gray-100">
                    <a href="{{ route('dashboard') }}" class="text-sm text-[#7c7e8c] hover:text-[#1e1f22] transition-colors border-b border-gray-300 hover:border-gray-600 pb-0.5">
                        Вернуться назад
                    </a>
                    <button type="submit"
                            class="w-full sm:w-auto bg-[#ff5c8a] text-white px-8 py-3.5 rounded-full text-sm  font-normal  hover:bg-[#e04b75] transition-colors duration-300 shadow-md shadow-pink-100 hover:cursor-pointer text-center">
                        Отправить отзыв
                    </button>
                </div>
            </form>
