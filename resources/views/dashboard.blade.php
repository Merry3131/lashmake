...
<x-app-layout>
    ...
    <div class="w-full bg-cover bg-center pt-16 pb-16 relative" style="background-image: url('{{ asset('img/bg_main.png') }}');">
        ...
        <div class="max-w-7xl mx-auto px-6 text-center">
            <h2 class="text-3xl lg:text-4xltext-[#1e1f22] tracking-widest mb-4 font-[Playfair_Display]">
                Личный кабинет
            </h2>
    ...
    <div class="fixed top-6 right-6 z-50 space-y-3 max-w-sm w-full font-sans" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)">
        @if(session('error'))
            <div class="bg-[#fff5f5] border border-[#ffe3e3] p-4 rounded-2xl shadow-lg flex items-center gap-3 text-sm text-[#1e1f22]">
                <div class="w-7 h-7 bg-[#ffe3e3] text-[#e53e3e] rounded-xl flex items-center justify-center flex-shrink-0 ">!</div>
                <p class="flex-grow font-light">{{ session('error') }}</p>
                <button @click="show = false" class="text-gray-400 hover:text-gray-600">×</button>
            </div>
        @endif

        @if(session('success'))
            <div class="bg-[#fff0f3] border border-pink-100 p-4 rounded-2xl shadow-lg flex items-center gap-3 text-sm text-[#1e1f22]">
                <div class="w-7 h-7 bg-pink-100 text-[#ff5c8a] rounded-xl flex items-center justify-center flex-shrink-0 ">✓</div>
                <p class="flex-grow font-light">{{ session('success') }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div class="bg-white rounded-3xl border border-[#f1f1f5] p-6 hover:shadow-sm transition-all duration-300">
                ...
                <div class="space-y-4 text-gray-700">
                    <div>
                        <span class="block text-sm  text-[#7c7e8c] font-medium mb-1 font-[Manrope]">Имя</span>
                    ...
                    <div>
                        <span class="block text-sm  text-[#7c7e8c] font-medium mb-1 font-[Manrope]">Фамилия</span>
                        <p class="text-sm text-[#1e1f22] font-light bg-[#f8f8fa] border border-[#f1f1f5] rounded-xl px-4 py-2 font-[Manrope]">
                            {{ Auth::user()->last_name }}
                        </p>
                    </div>
                    ...
                    <div>
                        <span class="block text-sm  text-[#7c7e8c] font-medium mb-1 font-[Manrope]">Телефон</span>
                    ...
                    <div>
                        <span class="block text-sm  text-[#7c7e8c] font-medium mb-1 font-[Manrope]">Электронная почта</span>
                ...
                <div class="pt-6 flex">
                    <a href="{{ route('profile.edit') }}" class="w-full border border-[#ff5c8a] text-[#ff5c8a] rounded-xl py-3 text-sm  font-normal hover:bg-[#ff5c8a] hover:text-white text-center transition-all duration-300 shadow-sm font-[Manrope]">
                        Редактировать
                    </a>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="space-y-4">
                    @forelse($appointments as $appointment)
                        <div class="group bg-white rounded-3xl border border-[#f1f1f5] p-6 flex flex-col gap-4 hover:shadow-sm transition-all duration-300">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-center gap-4 min-w-[140px]">
                                    <div class="w-12 h-12 bg-[#f8f8fa] border border-[#f1f1f5] rounded-2xl flex flex-col items-center justify-center text-center p-1 flex-shrink-0 group-hover:border-[#ff5c8a]/30 transition-colors">
                                        <span class="text-sm text-gray-400 font-light leading-none mb-0.5 font-[Manrope]">
                                            {{ \Carbon\Carbon::parse($appointment->appointment_at)->isoFormat('dd') }}
                                        </span>
                                        <span class="text-sm font-normal text-[#1e1f22] leading-none font-[Manrope]">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-sm text-[#7c7e8c] font-light flex items-center gap-1 font-[Manrope]">
                                ...
                                <div class="flex-grow space-y-1 sm:pl-4 sm:border-l border-gray-100">
                                    ...
                                    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[#7c7e8c] font-light">
                            ...
                            <div class="flex flex-col sm:flex-row items-end sm:items-center justify-between gap-4 pt-3 sm:pt-0 border-t sm:border-t-0 border-dashed border-gray-100">
                                ...
                                <div>
                                    @if($appointment->status === 'completed' && !$appointment->review)
                                        <a href="{{ route('reviews.create', ['appointment' => $appointment->id]) }}"
                                           class="inline-block border border-[#ff5c8a] text-[#ff5c8a] hover:bg-[#ff5c8a] hover:text-white rounded-xl py-2 px-4 text-sm tracking-wide font-normal transition-colors duration-300">
                                            Оставить отзыв
                                        </a>
                                    @elseif($appointment->status === 'completed' && $appointment->review)
                                        <span class="inline-flex items-center px-3 py-2 text-sm font-normal tracking-wide rounded-xl bg-gray-100 text-gray-500">
                                            Отзыв оставлен
                                        </span>
                                    @endif
                                </div>
